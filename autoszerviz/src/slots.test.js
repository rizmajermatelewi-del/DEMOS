...
describe('slotsForDate', () => {
  ...
  it('stops a service that would run past closing', () => {
    const slots = slotsForDate(TUE, 90, [], new Date('2026-09-22T07:00:00'))
    expect(slots[0]).toBe(7 * 60 + 30)
    expect(slots.at(-1)).toBe(15 * 60 + 30)
    expect(slots).not.toContain(16 * 60)
  })
  ...
  it('hides a start that has already passed', () => {
    ...
    expect(slots).not.toContain(7 * 60 + 30)
  })
  ...
  it('lets a service end exactly at Saturday closing, and not a step later', () => {
    ...
    expect(slots.at(-1)).toBe(11 * 60)
    expect(slots).not.toContain(11 * 60 + 30)
  })
